...
def pad_my(img: Image, divable: int = 16) -> Image:
    """Pad an Image to the next full divisible value of `divable`. Also normalizes the image and invert if needed.
    ...
    for x in [w, h]:
        div, mod = divmod(x, divable)
        # dims.append(divable*(div + (1 if mod > 0 else 0)))
        dims.append(divable * (div + 1))
    padded = Image.new('L', dims, 255)
    l_boarder = (dims[0] - w) // 2
    u_boarder = (dims[1] - h) // 2

    # padded.paste(img, (l_boarder, 0, img.size[0]+l_boarder, img.size[1]))
    padded.paste(img, (l_boarder, u_boarder))
    w, h = padded.size
    ...
    if w > max_w:
        r = w / max_w
        h_1 = int(h / r)
        div, mod = divmod(h_1, divable)
        h = divable * (div + (1 if mod > 0 else 0))
        # padded = padded.resize((max_w, h),Image.ANTIALIAS)
        padded = padded.resize((max_w, h),Image.LANCZOS)
    if h > max_h:
        r = h / max_h
        w_1 = int(w / r)
        div, mod = divmod(w_1, divable)
        w = divable * (div + (1 if mod > 0 else 0))
        padded = padded.resize((w, max_h),Image.LANCZOS)
    return padded

    # div_w, mod_w = divmod(w, divable)
    # w1 = (div_w+1)*divable
    # l_boarder = (w1 * 16 - w) // 2
    # r_boarder = w1 * 16 - w - l_boarder
    # img = cv2.copyMakeBorder(img, 0, 0, l_boarder, r_boarder, cv2.BORDER_CONSTANT, value=(255, 255, 255))
    #
    # div_h, mod_h = divmod(h, divable)
    # h1 = (div_h + 1) * divable
    # u_boarder = (h1 * 16 - h) // 2
    # b_boarder = h1 * 16 - h - u_boarder
    # img = cv2.copyMakeBorder(img, u_boarder, b_boarder, 0, 0, cv2.BORDER_CONSTANT, value=(255, 255, 255))
    # image = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    # # print("cv2.imshow_padded_img")
    # print("show_padded_img")
    # img.show()
    # return img
